...
(use-package treesit
  ...
  :config
  ;; Try to install grammar (safe if already installed)
  (when (fboundp 'treesit-install-language-grammar)
    (ignore-errors (treesit-install-language-grammar 'rust))))

;; Chooser: prefer rust-ts-mode; on error, fall back to rust-mode
(defun gjk/rust-open ()
  "Open Rust files using rust-ts-mode if possible; else rust-mode.
Echo which mode was chosen."
  (interactive)
  (if (featurep 'treesit)
      (condition-case err
          (progn
            (require 'rust-ts-mode) ; built-in in Emacs 29
            (rust-ts-mode)
            (message "✅ Using rust-ts-mode (tree-sitter)"))
        (error
         (require 'rust-mode nil t)
         (rust-mode)
         (message "❌ rust-ts-mode failed (%s) → using rust-mode"
                  (error-message-string err))))
    (progn
      (require 'rust-mode nil t)
      (rust-mode)
      (message "ℹ️ treesit not available → using rust-mode"))))

;; Route .rs through the chooser (keeps the rest of your setup intact)
(add-to-list 'auto-mode-alist '("\\.rs\\'" . gjk/rust-open))

;;; --- Rust major mode (tree-sitter first, auto-fallback to classic) ---
(use-package rust-ts-mode
  :ensure nil                  ;; built-in on Emacs 29
  ;; :mode "\\.rs\\'"          ;; handled by chooser above
  :init
  ;; Informative message at init time only (actual choice happens per file)
  (when (featurep 'treesit)
    (message "ℹ️ Tree-sitter present; will try rust-ts-mode for .rs files"))
  :hook
